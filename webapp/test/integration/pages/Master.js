			...
			actions : {
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("idocNumber") === oObject.idocNumber;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].idocNumber;
									} else {
										sCurrentId = aItemsNotInTheList[0].idocNumber;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("openAdjustmentSet", {
										idocNumber : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("idocNumber"),
						title: oBindingContext.getProperty("idocNumber")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						viewName : sViewName,
						id : "page",
						matchers : new PropertyStrictEquals({name : "title", value : "<openAdjustmentSet> (0)"}),
						success : function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("idocNumber");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all openAdjustmentSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "page",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "title", value : "<openAdjustmentSet> (" + iExpectedLength + ")"}),
								success : function () {
